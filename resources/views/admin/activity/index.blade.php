...
<section class="min-h-[90vh] w-full">
    <div class="breadcrumbs px-0 ">
        <ul class="mx-0 px-0 my-0">
          <li class="mx-0 px-0">Kegiatan</li> 
          <li>List</li>
        </ul>
    </div>
    <div class="w-full flex justify-between items-center mb-4">
        <h2 class="font-semibold my-4">Kegiatan</h2>
        <a 
    ...
    <div class="bg-base-100 p-4 rounded-xl shadow-xl">
        <div class="w-full mb-4 flex flex-col lg:flex-row justify-end gap-4">
            <form action="{{ route('admin.activity') }}" method="GET" class="flex items-center gap-2">
                @csrf
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <img src="{{ asset('imageActivity/'.$item->image) }}" alt="{{ $item->title }}" class="max-w-[100px] rounded mx-auto">
                        </td>
                        <td>{{ $item->title }}</td>
                        ...
                        <td>
                            <div class="flex justify-center items-center gap-2">
                                <a href="{{ url('admin/activity/'.$item->id).'/edit'}}" class="btn btn-square btn-warning btn-cta">
                    ...
                    <dialog id="modalConfirm{{ $item->id }}" class="modal bg-base-200 bg-clip-padding backdrop-filter backdrop-blur-md bg-opacity-10">
                        <div class="modal-box prose-lg">
                            ...
                            <div class="modal-action">
                                ...
                                <form action="{{ url('admin/activity/'.$item->id)}}" method="POST">
                                    @csrf 
                                    @method('DELETE')
