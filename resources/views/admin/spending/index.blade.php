...
@section('title', "Daarul Rahmah - Cash")
@section('content')
<section class="min-h-[90vh] w-full">
    <div class="breadcrumbs px-0 ">
        <ul class="mx-0 px-0 my-0">
          <li class="mx-0 px-0">Pengeluaran</li> 
          <li>List</li>
        </ul>
    </div>
    <div class="w-full flex justify-between items-center mb-4">
        <h2 class="font-semibold my-4">Pengeluaran</h2>
        @if ((auth()->user()->role == 'admin') || (auth()->user()->role == 'super-admin'))
        <a 
        href="{{ route('admin.spending.create')}}"
    ...
    <div class="bg-base-100 p-4 rounded-xl shadow-xl">
        <div class="w-full mb-4 flex flex-col lg:flex-row justify-end gap-4">
            <a href="{{ route('admin.spending.export') }}" class="btn btn-secondary btn-cta ml-auto">Export CSV</a>
            <div class="divider divider-horizontal mx-0 hidden lg:flex"></div>
            <form action="{{ route('admin.spending') }}" method="GET" class="flex items-center gap-2">
                @csrf
                <input type="text" name="keyword" value="{{ Request::get('keyword') }}" class="input input-bordered w-full" placeholder="Cari...">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <div class="flex justify-center items-center gap-2">
                                <a href="{{ url('admin/spending/'.$item->id).'/edit'}}" class="btn btn-square btn-warning btn-cta">
                    ...
                    <dialog id="modalConfirm{{ $item->id }}" class="modal bg-base-200 bg-clip-padding backdrop-filter backdrop-blur-md bg-opacity-10">
                        <div class="modal-box prose-lg">
                            ...
                            <div class="modal-action">
                                ...
                                <form action="{{ url('admin/spending/'.$item->id)}}" method="POST">
                                    @csrf 
                                    @method('DELETE')
